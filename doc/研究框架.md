...
![参数扫描箱型图](../exp/analysitic_exp/figs/known_scan_box.png)

*图 1. 第一类系统参数扫描奇异值谱箱型图。用于说明不同参数组合下谱分离的强弱。*

![固定参数相图](../exp/analysitic_exp/figs/known_phase.png)

*图 2. 第一类系统固定参数轨迹相图。用于展示该系统在状态空间中的主收缩结构。*

![干净与含噪轨迹](../exp/analysitic_exp/figs/known_clean_noisy.png)
...
![解析矩阵 A](../exp/analysitic_exp/figs/known_A.png)

*图 4. 第二部分解析矩阵 $A$ 热力图。用于展示已知确定动力学在观测层的线性结构。*

![解析矩阵奇异值谱](../exp/analysitic_exp/figs/known_A_spectrum.png)

*图 5. 第二部分解析矩阵奇异值谱图。用于观察矩阵级谱分离和主方向强度。*

![解析矩阵粗粒化矩阵](../exp/analysitic_exp/figs/known_W.png)

*图 6. 第二部分由解析矩阵直接构造得到的粗粒化矩阵 $W$ 热力图。*

第三部分重点给出完整两步 SVD 的核心图：

![观测层协方差矩阵](../exp/analysitic_exp/figs/known_micro_sigma.png)

*图 7. 第三部分观测层协方差矩阵 $\Sigma_o$ 热力图。该矩阵由观测层噪音样本直接构造。*

![第二次分解矩阵](../exp/analysitic_exp/figs/known_stage2_matrix.png)

*图 8. 完整两步 SVD 中第二次分解矩阵 $\bar U\bar S$ 的热力图。它对应第一次截断后保留方向的加权组合。*

![第二次分解奇异值谱](../exp/analysitic_exp/figs/known_stage2_spectrum.png)

*图 9. 完整两步 SVD 中第二次分解的奇异值谱图。用于说明最终宏观维数如何由第二次截断确定。*

![宏微观曲线对比](../exp/analysitic_exp/figs/known_macro_curve.png)

*图 10. 第三部分宏微观曲线对比图。用于观察一维宏观变量是否保留了微观层主要动力学趋势。*

第四部分给出 `step2` 系统上的方法对照图：

![step2 谱对比](../exp/analysitic_exp/figs/step2_spectrum_compare.png)

*图 11. `step2` 系统奇异值谱与特征值谱对比图。说明 SVD 与 EVD 强调的结构层面不同。*

![step2 SVD 的 W](../exp/analysitic_exp/figs/step2_W_svd.png)

*图 12. `step2` 系统中 SVD 路线得到的粗粒化矩阵 $W$。*

![step2 EVD 的 W](../exp/analysitic_exp/figs/step2_W_evd.png)

*图 13. `step2` 系统中 EVD 路线得到的粗粒化矩阵 $W$。*

![step2 曲线对比](../exp/analysitic_exp/figs/step2_curve_compare.png)
